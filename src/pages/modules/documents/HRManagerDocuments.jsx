...
export default function HRManagerDocuments() {
    ...
    const handleRejectDoc = useCallback(async (docId, userId, docTitle) => {
        ...
        try {
            ...
            const { data: docData } = await supabase.from('documents').select('file_url').eq('id', docId).single();

            // 2. Notify employee BEFORE deletion so they know why it was rejected
            ...
            if (docData?.file_url) {
                await supabase.storage.from('documents').remove([docData.file_url]);
            }

            // 4. Delete the record from the database
            await supabase.from('documents').delete().eq('id', docId);

            flash(`${DOC_LABELS[docTitle] || docTitle} rejected and removed — employee notified`);
            setShowRejectInput(prev => ({ ...prev, [docId]: false }));
            setRejectComment(prev => ({ ...prev, [docId]: '' }));
            cacheService.invalidatePattern('^doc');
            await fetchData();
        } catch (err) { flash('Operation failed: ' + err.message, 'error'); }
        setActionLoading(prev => ({ ...prev, [docId]: false }));
    ...
    return (
        <>
            <div className="space-y-6 animate-fade-in">
                ...
                {toast.msg && (
                    <div className={`flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-medium animate-fade-in ${toast.type === 'error' ? 'bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400' : 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400'
                        }`}>
                                            ...
                                            <div key={sub.userId} className="bg-surface-primary rounded-2xl border border-border-secondary overflow-hidden h-fit">
                                                ...
                                                <div className="p-5 space-y-3">
                                                    {sub.docs.map(doc => {
                                                        ...
                                                        return (
                                                            <div key={doc.id} className={`p-3 rounded-xl border flex flex-col gap-2 ${isApproved ? 'bg-emerald-500/5 border-emerald-500/20'
                                                                ...
                                                                <div className="flex items-center justify-between">
                                                                    <div className="flex flex-col gap-0.5">
                                                                        ...
                                                                        <span className={`text-[10px] font-semibold flex items-center gap-1 ${isApproved ? 'text-emerald-600 dark:text-emerald-400' : isRejected ? 'text-red-600 dark:text-red-400' : 'text-brand-500'
                                                                            }`}>
