...
export default function AdminPerformanceView() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // 3. Fetch Admin Notes for managers/employees
            let notesByUser = {};
            try {
                const { data: adminNotesData = [] } = await supabase
                    .from('admin_notes')
                    .select('id, assigned_to, note, created_at, author_user_id, author_role')
                    .eq('entreprise_id', profile.entreprise_id)
                    .order('created_at', { ascending: false });
                notesByUser = adminNotesData.reduce((acc, note) => {
                    if (!note.assigned_to) return acc;
                    if (!acc[note.assigned_to]) acc[note.assigned_to] = [];
                    acc[note.assigned_to].push({ ...note, text: note.note });
                    return acc;
                }, {});
            } catch (notesErr) {
                console.warn('[AdminPerformanceView] admin_notes table unavailable, continuing without notes', notesErr?.message);
            }
            ...
            const calculatedManagers = managerRoleUsers.map(mgr => {
                ...
                return {
                    ...
                    notes: notesByUser[mgr.id] || []
                };
            });

            setManagers(prev => calculatedManagers.map(newMgr => {
                const existing = prev.find(p => p.id === newMgr.id);
                return {
                    ...newMgr,
                    notes: newMgr.notes.length ? newMgr.notes : (existing?.notes || []),
                };
            }));
    ...
    const handleSaveNote = async () => {
        ...
        const basePayload = {
            ...
            author_role: (profile?.role || '').toUpperCase().includes('HR') ? 'HR' : 'ADMIN',
            note: noteText.trim()
        };
        ...
        if (editingNoteId) {
            try {
                ...
                const newNotes = selectedManager.notes.map(item => item.id === editingNoteId
                    ? { ...item, note: basePayload.note, text: basePayload.note }
                    : item)

                setManagers(prev => prev.map(m => {
                    if (m.id !== selectedManager.id) return m;
                    return {
                        ...m,
                        notes: newNotes,
                    };
            ...
            try {
                ...
                notificationService.send(
                    selectedManager.id,
                    `New admin evaluation note: ${noteText}`,
                    'info',
                    {
                        source: 'admin_evaluation_note',
                        author_id: profile?.id,
                        author_name: profile?.name || profile?.email || 'Admin',
                        note_id: insertedNote?.id,
                    }
                );

                const normalizedNote = insertedNote ? { ...insertedNote, text: insertedNote.note } : null;
                setManagers(prev => prev.map(m => {
                    if (m.id !== selectedManager.id) return m;
                    return {
                        ...m,
                        notes: normalizedNote ? [normalizedNote, ...m.notes] : m.notes,
                    };
                }));
            } catch (err) {
                console.error('Failed to create admin note', err);
            }
        }
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="col-span-1">
                            <div className="bg-surface-primary rounded-2xl border border-border-secondary p-6 shadow-sm h-full flex flex-col">
                                ...
                                {!selectedManager ? (
                                    <div className="flex-1 flex flex-col mt-4 overflow-y-auto pr-2 space-y-3">
                                        {managers.flatMap(m => m.notes || []).length === 0 ? (
                                            <div className="flex-1 flex flex-col items-center justify-center text-center p-6 text-text-tertiary border-2 border-dashed border-border-secondary rounded-xl h-full">
                                                <Edit3 size={32} className="mb-3 opacity-50" />
                                                <p className="text-sm font-medium">No notes recorded yet.</p>
                                                <p className="text-xs mt-1">Select a manager to add notes.</p>
                                            </div>
                                        ) : (
                                            managers.flatMap(m => (m.notes || []).map(n => ({ ...n, managerName: m.name })))
                                                ...
                                                .map(note => (
                                                    <div key={note.id} className="p-3 bg-surface-secondary rounded-xl border border-border-secondary group relative">
                                                        <div className="flex justify-between items-start mb-1">
                                                            <span className="text-xs font-bold text-brand-600">To: {note.managerName}</span>
                                                            <span className="text-[10px] text-text-tertiary flex items-center gap-1">
                                                                <Clock size={10} /> {new Date(note.created_at || new Date()).toLocaleString()}
                                                            </span>
                                                        </div>
