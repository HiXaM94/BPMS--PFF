...
const emptyPostForm = { title: '', salary: '', contract_type: 'Full-time', description: '' };
const emptyEditForm = { title: '', department: 'Engineering', location: 'Casablanca', type: 'Full-time', salaryMin: '', salaryMax: '', description: '' };
...
export default function Recruitment() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setJobs((jobsData || []).map(j => ({
      ...
      department: j.department_id || '-',
      location: j.location || '-',
      type: j.employment_type || 'full_time',
      applicants: j.applicants_count ?? 0,
      shortlisted: j.shortlisted_count ?? 0,
      status: j.status || 'draft',
      postedDate: fmtDate(j.created_at),
      salary: j.salary_min && j.salary_max ? `${j.salary_min}-${j.salary_max} MAD` : 'Negotiable',
      description: j.description || '',
      requirements: j.requirements || '',
      closing_date: j.closed_at,
    })));
  ...
  const handlePost = async (e) => {
    ...
    if (!isSupabaseReady) {
      setJobs(prev => [{
        id: Date.now(), title: form.title, department: '-', location: '-',
        type: form.contract_type, applicants: 0, shortlisted: 0, status: 'open',
        postedDate: fmtDate(new Date()),
        salary: form.salary || 'Negotiable',
        description: form.description,
      }, ...prev]);
      setForm(emptyPostForm); setShowNew(false); setSaving(false);
      flash('Job posted successfully!');
      return;
    }
    const { error } = await supabase.from('recrutements').insert([{
      enterprise_id: '11111111-0001-0001-0001-000000000000',
      title: form.title,
      description: form.description,
      requirements: `Salary: ${form.salary} | Contract: ${form.contract_type}`,
    }]);
    setSaving(false);
    if (error) { flash('Error: ' + error.message); return; }
    setForm(emptyPostForm); setShowNew(false);
    flash('Job posted successfully!');
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Post New Job Modal */}
      <Modal isOpen={showNew} onClose={() => { setShowNew(false); setForm(emptyPostForm); }} title="Post New Job" maxWidth="max-w-xl"
        footer={
        ...
        <form id="job-form" onSubmit={handlePost} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required value={form.salary} onChange={e => setForm(f => ({ ...f, salary: e.target.value }))} placeholder="e.g. 15000 MAD" className={inp}/>
            </div>
            <div>
              <label className={lbl}>Contract Type *</label>
              <select required value={form.contract_type} onChange={e => setForm(f => ({ ...f, contract_type: e.target.value }))} className={inp + ' cursor-pointer'}>
          ...
          <div>
            ...
            <textarea required rows={4} value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Describe the role and responsibilities..." className={inp + ' resize-none'}/>
          </div>
        </form>
      </Modal>
