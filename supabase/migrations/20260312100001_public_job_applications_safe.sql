-- Migration: Public Job Applications (Safe version - can be run multiple times)
...
DROP POLICY IF EXISTS "HR can delete CVs" ON storage.objects;
DROP POLICY IF EXISTS "Anyone can view public open jobs" ON recrutements;
DROP POLICY IF EXISTS "Anyone can submit applications" ON candidates;
DROP POLICY IF EXISTS "HR and Admin can view all candidates" ON candidates;
DROP POLICY IF EXISTS "HR and Admin can update candidates" ON candidates;
DROP POLICY IF EXISTS "HR and Admin can delete candidates" ON candidates;
...
DROP FUNCTION IF EXISTS decrement_applicant_count();
...
-- Create index for public job queries (only if it doesn't exist)
